...
- **Station Health Scaling Crashes:** Fixed a critical stack trace error in `cw_frontlinesiege` and `cw_decapitationstrike` where the script illegally attempted to overwrite the read-only `maxDurability` property on the generated stations. The scaling mechanism has been safely shifted to use Avorion's native `damageMultiplier` and boarding `defenseMultiplier`, ensuring the missions run smoothly and scale difficulty safely without crashing.
- **False Mission Completion (No Payout Bug):** Fixed a bug in `cw_borderskirmish`, `cw_decapitationstrike`, `cw_frontlinesiege`, `cw_interception`, and `cw_resourcesabotage` where the player would seemingly complete the mission instantly but receive no payment. This was caused by the mission's phase triggers checking for remaining enemies on the client. Because entity script variables are not synchronized to clients, the client always assumed 0 enemies were alive, triggering an instant local completion. Players who then left the sector never received their payment because the server had not yet verified the kills. The conditions are now strictly evaluated on the server.
