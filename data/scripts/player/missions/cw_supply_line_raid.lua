...
mission._Debug = 0
mission._Name = "War Contract: Supply Line Raid"
...
function initialize(factionIndex)
    if onServer() and not _restoring then
        ...
        mission.data.description = {
            ...
            { text = "An enemy logistics convoy is vulnerable in sector (${location.x}:${location.y}). Destroy the freighters before they warp out."%_T, arguments = { location = mission.data.location } },
        ...
        -- v4.0.0: realigned to the 0.80 War Heat tier band (was using the 0.25-tier
        -- formula copy-pasted from cw_borderskirmish.lua). Matches Blockade Runner,
        -- the nearest same-tier anchor for a multi-target strike under a timer.
...
function getBulletin(station)
    ...
    return {
        ...
        msg = "Cut off their supply lines. Leave no survivors."%_T,
        onAccept = [[
            local self, player = ...
