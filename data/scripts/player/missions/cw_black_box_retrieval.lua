...
mission._Debug = 0
mission._Name = "War Contract: Black Box Retrieval"
...
function initialize(factionIndex)
    if onServer() and not _restoring then
        ...
        mission.data.description = {
            ...
            { text = "Find our destroyed flagship in sector (${location.x}:${location.y}) and extract the black box data."%_T, arguments = { location = mission.data.location } },
            { text = "Head to sector (${location.x}:${location.y})"%_T, bulletPoint = true, fulfilled = false },
            { text = "Fly within 2.5km of the flagship wreckage to download the data"%_T, bulletPoint = true, fulfilled = false, visible = false }
        }
...
mission.phases[1].triggers = {
    {
        condition = function()
            ...
            for _, wreckage in pairs(wreckages) do
                if wreckage:getValue("cw_black_box_target") then
                    local distance = craft:getNearestDistance(wreckage)
...
function getBulletin(station)
    ...
    return {
        brief = "War Contract: Black Box Retrieval"%_T,
        description = "One of our prototype flagships was ambushed and destroyed in enemy territory. The enemy has dispatched salvage crews to strip the tech. Get in there, find the wreckage, and recover the black box data before they do.\n\nWARNING: Accepting this contract is an act of war. You will immediately become hostile to the target faction."%_T,
        difficulty = "Hard"%_T,
        reward = "¢${reward}"%_T,
        script = "data/scripts/player/missions/cw_black_box_retrieval.lua",
        icon = "data/textures/icons/ShipRecon.png",
        formatArguments = { reward = createMonetaryString(rewardCredits) },
        arguments = { { giver = station.factionIndex, reward = rewardStruct } },
        msg = "Get that data back at all costs. Dismissed."%_T,
        onAccept = [[
            local self, player = ...
