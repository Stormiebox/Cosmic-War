...
local __CW_refine_calculatePrediction_original = RefineCommand.calculatePrediction

function RefineCommand:calculatePrediction(ownerIndex, shipName, area, config)
    local prediction = __CW_refine_calculatePrediction_original(self, ownerIndex, shipName, area, config)
